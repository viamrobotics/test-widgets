...
const colorMapGrey = [
	...
	[10, 10, 10],
	[0, 0, 0],
].map(([red, green, blue]) => new Vector3(red, green, blue).multiplyScalar(1 / 255))
const USER_GENERATED_POINT_COLOR = new Color(theme.extend.colors.cyberpunk)
 ...
 * ticket to add testing: https://viam.atlassian.net/browse/RSDK-2606
 */
const colorBuckets = (probability: number): Vector3 => {
	const bucket = probToColorMapBucket(probability, colorMapGrey.length)
	return colorMapGrey[bucket] ?? new Vector3()
}

export const mapColorAttributeGrayscale = (colors: BufferAttribute) => {
